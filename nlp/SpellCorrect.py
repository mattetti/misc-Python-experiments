...
import types
from LanguageModelMedley import LanguageModelMedley
  ...
  def correctSentence(self, sentence):
    ...
    for i in range(1, len(sentence) - 1):
      ...
      for alternative, editscore in editProbs.iteritems():
        ...
        if self.classModel:
          lmscore = self.classModel.score(sentence)
        else:
          lmscore = getattr(self.languageModels, self.languageModelFunction)(sentence)
        if editscore != 0:
...
def main():
  ...
  models = LanguageModelMedley(trainingCorpus)

  print 'Uniform Language Model: '
  uniformSpell = SpellCorrect(models, 'uniform', trainingCorpus)
  uniformOutcome = uniformSpell.evaluate(devCorpus) 
  print str(uniformOutcome)

  print 'Laplace Unigram Language Model: '
  laplaceUnigramSpell = SpellCorrect(models, 'laplaceUnigram', trainingCorpus)
  ...
  laplaceBigramSpell = SpellCorrect(models, 'laplaceBigram', trainingCorpus)
  ...
  sbSpell = SpellCorrect(models, 'stupidBackOff', trainingCorpus)
  sbOutcome = sbSpell.evaluate(devCorpus)
  print str(sbOutcome)

  print 'Custom Language Model: '
  customSpell = SpellCorrect(models, 'kneserNey', trainingCorpus)
